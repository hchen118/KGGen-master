...
class ScaledDotProductAttention():
	...
	def __call__(self, q, k, v, mask):   # mask_k or mask_qk
		...
		temper = np.sqrt(20)		#just fix the bug, dk = 20
...
class SelfAttention():
	...
	def __call__(self, src_emb, return_att=False, active_layers=999):
		if return_att: atts = []
		mask = None     #no mask in our model
		x = src_emb		
...
class Decoder():
	...
	def __call__(self, relation_emb, enc_output, return_att=False, active_layers=999):
		x = relation_emb
...
class Transformer:
	def __init__(self, len_limit, d_model=256, \
		...
		self.target_layer = TimeDistributed(Dense(d_inner_hid, use_bias=False))   # I don't know what's the meaning of this layer

	def generate_layer(self, src_emb, relation_emb, active_layers=999):
		...
		# relation_emb = self.o_word_emb(tgt_seq)

		# if self.pos_emb:
			# intermediate_dim = src_emb.shape[2]
			# src_emb_merge = Reshape((2 * intermediate_dim,))(src_emb)  # shape(2,1000)->shape(2000)
			# src_emb = add_layer([src_emb, self.pos_emb(src_emb_merge)])
			# relation_emb = add_layer([relation_emb, self.pos_emb(tgt_seq)])
		src_emb = self.emb_dropout(src_emb)

		# enc_output = Dense(100)(src_emb)
		# enc_output = src_emb
		enc_output = self.encoder(src_emb, active_layers=active_layers)

		dec_output = self.decoder(relation_emb, enc_output, active_layers=active_layers)
	...
	def generate_Encoder_layer(self, src_emb, active_layers=999):
	...
	def generate_Decoder_layer(self, enc_output, relation_emb, active_layers=999):
